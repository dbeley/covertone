...
{#if !isDesktop}
  <div class="fixed inset-0 z-[60] flex items-end justify-center animate-fade-in md:hidden">
    ...
    ></button>
    <div
      class="relative w-full max-w-lg bg-surface border border-border border-b-0 rounded-t-2xl max-h-[85vh] flex flex-col animate-slide-up shadow-2xl shadow-black/20"
      style="padding-bottom: var(--safe-area-inset-bottom); transform: translateY({dragY}px); transition: {dragging ? 'none' : 'transform 0.3s ease-out'}"
    ...
    >
      <div class="shrink-0 flex items-center justify-between px-5 pt-4 pb-2">
        <span class="text-xs text-text-dim font-medium">Now Playing</span>
        <button class="p-2 rounded-xl hover:bg-white/5 text-text-dim hover:text-text transition-all duration-150 active:scale-90" onclick={onClose} aria-label="Close">
      ...
      <div class="flex-1 overflow-y-auto px-5 pb-6">
        {#if currentTrack}
          <div class="flex flex-col items-center gap-5">
            <LazyImage
              ...
              class="w-48 h-48 rounded-2xl object-cover shadow-2xl shadow-black/20 ring-1 ring-border/50"
            />

            <div class="text-center w-full">
              <h1 class="text-lg font-bold truncate">{currentTrack.title}</h1>
        ...
        {:else}
          <p class="text-text-dim text-center py-8">Nothing playing</p>
        {/if}
      </div>
    </div>
  </div>

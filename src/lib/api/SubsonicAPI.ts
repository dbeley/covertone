...
export class SubsonicAPI {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (data["subsonic-response"].status === "failed") {
        throw new Error(
          data["subsonic-response"].error?.message ?? "Unknown error",
        );
      }
      return data;
    } catch (e) {
  ...
  getCoverArt(params: { id: string; size?: number }): string {
    return this.buildUrl(ENDPOINTS.getCoverArt, {
      id: params.id,
      size: params.size,
    });
  }
  ...
  async scrobble(params: {
    ...
    await this.request(ENDPOINTS.scrobble, {
      id: params.id,
      time: params.time,
...
export function getCoverArtUrl(config: {
  server: string;
  username: string;
  password: string;
  id: string;
...
export function getStreamBaseUrl(config: {
  server: string;
  username: string;
  password: string;
}): string {
...
export async function scrobbleTrack(config: {
  ...
  try {
    ...
    console.warn("Scrobble request failed:", e);
  }
}

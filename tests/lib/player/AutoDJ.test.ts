...
function createMockApi(getSimilarSongsImpl?: Function, getRandomSongsImpl?: Function) {
  return {
    getSimilarSongs: vi.fn(getSimilarSongsImpl ?? (() => Promise.resolve({ similarSongs: { song: [] } }))),
    getRandomSongs: vi.fn(getRandomSongsImpl ?? (() => Promise.resolve({ randomSongs: { song: [] } }))),
  };
}
...
describe('AutoDJ', () => {
  it('fetchSimilar returns similar songs when available', async () => {
    ...
    expect(mockApi.getSimilarSongs).toHaveBeenCalledWith({ id: '1', count: 10 });

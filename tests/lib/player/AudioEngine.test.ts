...
function createMockAudio() {
  ...
  const audio = {
    ...
    pause: vi.fn().mockImplementation(() => { audio.paused = true; }),
    load: vi.fn(),
  };
...
describe('AudioEngine', () => {
  ...
  beforeEach(() => {
    ...
    AudioSpy = vi.spyOn(window, 'Audio').mockImplementation(() => mock.audio as any);
  ...
  it('registers event listeners in constructor', () => {
    expect(mock.audio.addEventListener).toHaveBeenCalledWith('timeupdate', expect.any(Function));
    expect(mock.audio.addEventListener).toHaveBeenCalledWith('ended', expect.any(Function));
    expect(mock.audio.addEventListener).toHaveBeenCalledWith('loadedmetadata', expect.any(Function));
  });

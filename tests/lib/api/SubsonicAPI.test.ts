...
describe("SubsonicAPI", () => {
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({
              directory: { id: "1", name: "Album", child: [mockSong] },
            }),
          ),
        ),
    });
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({
              artists: { index: [{ name: "A", artist: [mockArtist] }] },
            }),
          ),
        ),
    });
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({
              artist: { id: "a1", name: "Artist", album: [mockAlbum] },
            }),
          ),
        ),
    });
          ...
          JSON.stringify(
            createSubsonicResponse({
              artistInfo2: {
                ...
                similarArtist: [{ id: "a2", name: "Similar Artist" }],
              },
            }),
          ),
        ),
  ...
  it("getTopSongs fetches top songs for artist", async () => {
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({ topSongs: { song: [mockSong] } }),
          ),
        ),
    });
  ...
  it("getRandomSongs fetches random songs", async () => {
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({ randomSongs: { song: [mockSong] } }),
          ),
        ),
    });
  ...
  it("getSimilarSongs fetches similar songs", async () => {
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({ similarSongs: { song: [mockSong] } }),
          ),
        ),
    });
    ...
    fetchSpy.mockResolvedValueOnce({
      ...
      text: () =>
        Promise.resolve(
          JSON.stringify(
            createSubsonicResponse({
              searchResult3: { song: [mockSong], album: [], artist: [] },
            }),
          ),
        ),
    });
  ...
  it("caches responses and returns cached data on second call", async () => {
    ...
    const result1 = await api.getAlbumList({ type: "newest", size: 20 });
    expect(result1.albumList2.album).toHaveLength(1);
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    // Second call should use cache
    const result2 = await api.getAlbumList({ type: "newest", size: 20 });
    expect(result2.albumList2.album).toHaveLength(1);
    expect(fetchSpy).toHaveBeenCalledTimes(1); // still 1
  });

  it("invalidates cache after star", async () => {
    const mockAlbum: Album = {
      id: "1",
      name: "Album",
      artist: "Artist",
    ...
    // Cache an album list call
    await api.getAlbumList({ type: "newest", size: 20 });
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    // Star invalidates cache
    await api.star({ id: "s-1" });
    // Next getAlbumList should fetch again
    await api.getAlbumList({ type: "newest", size: 20 });
    expect(fetchSpy).toHaveBeenCalledTimes(3); // first get + star + second get
  });
});

...
describe("library store", () => {
  ...
  beforeEach(() => {
    ...
    mockApi = {
      ...
      getAlbum: vi
        .fn()
        .mockResolvedValue({
          directory: { id: "1", name: "Album", child: [] },
        }),
      getArtists: vi
        .fn()
        .mockResolvedValue({
          artists: {
            index: [
              {
                name: "A",
                artist: [{ id: "a1", name: "Artist", albumCount: 3 }],
              },
            ],
          },
        }),
  ...
  it("init sets API instance", () => {
    ...
    expect(SubsonicAPI).toHaveBeenCalledWith({
      server: "https://example.com",
      username: "u",
      password: "p",
  ...
  it("fetchAlbums appends on subsequent calls", async () => {
    ...
    mockApi.getAlbumList.mockResolvedValueOnce({
      albumList2: { album: [mockAlbum] },
    });
    await library.fetchAlbums({ type: "newest" });
    const album2 = { ...mockAlbum, id: "2", name: "Album 2" };
    mockApi.getAlbumList.mockResolvedValueOnce({
      albumList2: { album: [album2] },
    });

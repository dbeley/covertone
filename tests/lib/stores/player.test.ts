...
const mockEngine = {
  load: vi.fn(),
  play: vi.fn().mockImplementation(() => { mockPaused = false; }),
  pause: vi.fn().mockImplementation(() => { mockPaused = true; }),
  toggle: vi.fn(),
  seek: vi.fn(),
  setVolume: vi.fn(),
  ...
  onEnded: vi.fn(),
  onLoaded: vi.fn(),
  destroy: vi.fn().mockImplementation(() => { mockPaused = true; }),
};
...
describe('player store', () => {
  ...
  it('setStreamBase stores the stream base URL', () => {
    ...
    expect(mockEngine.load).toHaveBeenCalledWith('https://example.com/rest/stream?id=1');
  });
  ...
  it('engine timeupdate callback updates store', () => {
    ...
    mockEngine.onTimeUpdate.mockImplementation((cb: Function) => { timeCb = cb; });
  ...
  it('engine onLoaded callback updates store', () => {
    ...
    mockEngine.onLoaded.mockImplementation((cb: Function) => { loadedCb = cb; });
  ...
  it('engine onEnded callback sets status to ended/idle', () => {
    ...
    mockEngine.onEnded.mockImplementation((cb: Function) => { endedCb = cb; });
